...
但是对于 stress tensor $T = \sum_{n\in \mathbb{Z}} L_n z^{-n - 2}$，
$$
L_{[n]} = \sum_{j \ge n}c(j, n; 2)L_j - \frac{c}{24}\delta_{j, -2} \ .
$$
...
则有
$$
\operatorname{str}_M\, o(a_{[-h_a]}b)\, x^{J_0} q^{L_0}
=
\delta_{Q,0}\,\operatorname{str}_M\, o(a)o(b)\, x^{J_0} q^{L_0} x^Q
+
\sum_{n=1}^{+\infty}
E_n\!\left[\begin{matrix} e^{2\pi i h_a} \\ x^Q \end{matrix}\right]
\operatorname{str}_M\, o(a_{[-h_a+n]}b)\, x^{J_0} q^{L_0}.
$$
其中 $\delta_{Q, 0}$ 是 Kronecker $\delta$，$o(a)$ 是 $a$ 的 zero-mode，$o(a) = a_0$ 是 $a$ 的 zero-mode。

并且，当 $n \gt 0$ 时，
$$
\operatorname{str}_M\, o(a_{[-h_a-n]}|b\rangle)\, q^{L_0} x^{J_0}
=
(-1)^n
\sum_{k=1}^{+\infty}
\binom{k-1}{n}
E_k\!\left[\begin{matrix} e^{2\pi i h_a} \\ x^Q \end{matrix}\right]
\operatorname{str}_M\, o(a_{[-h_a-n+k]}|b\rangle)\, q^{L_0} x^{J_0} \ ,
$$
其中 $x^h = $